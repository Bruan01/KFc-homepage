...
- K 股市：轮次制虚拟行情与固定资金机器人做市，带每日/单笔/持仓限额
- 后台登录、产品新增/编辑/删除
- 代码包上传并绑定产品
...
- `/market`：查看服务器行情、买卖虚拟资产、持仓和交易记录；访客可以查看行情，交易必须登录。
- `/admin`：主管理员可创建市场轮次和资产、配置机器人库存、追加有限资金、暂停/恢复/结算轮次，并查看订单、资金和后台审计。

股市不会通过奖励逻辑增发 K 积分。用户侧每次余额变化都经过 `apps.points.services.apply_ledger`；机器人资金只来自创建轮次时的初始预算或主管理员显式追加，资金、库存、预留兑付和每日支付额度不足时会拒绝交易。轮次结算会写入结算订单、积分流水、资金流水并归还机器人库存。
