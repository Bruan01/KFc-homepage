(function () {
  ...
  async function poll(id) { try { const response = await fetch(`/api/imaging/generations/${id}`); if (!response.ok) throw new Error("无法读取任务状态"); const job = await response.json();
      if (job.status === "queued" || job.status === "generating") { setState(job.status === "queued" ? "排队准备中" : "正在生成", "working"); say(`CPA 正在显影画面，已等待 ${Math.floor((Date.now() - startedAt) / 1000)} 秒。`); return; }
  ...
  async function submit(event) { event.preventDefault(); const text = prompt.value.trim(); if (!text) { say("先写下一句画面描述。", true); prompt.focus(); return; }
    ...
    try { const response = await fetch("/api/imaging/generations", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ prompt: text, size: $("#size").value, quality: $("#quality").value, output_format: $("#output-format").value, idempotencyKey: key() }) });
      ...
      $("#balance").textContent = data.balance; await poll(data.id); if (data.status === "queued" || data.status === "generating") timer = setInterval(() => poll(data.id), 2500);
  ...
  updateCount(); loadAccount().catch(() => {}); loadHistory().catch(() => {});
}());
