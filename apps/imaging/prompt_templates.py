"""Curated, parameterised prompt recipes used by the public imaging studio.

Templates are deliberately source-controlled for the first release: every recipe is
reviewable, versioned, and can evolve without exposing raw prompt construction to
the browser.  A database-backed admin editor can be layered on top later.
"""
...
class TemplateError(ValueError):
    """Raised when a template selection or one of its fields is invalid."""
...
def list_template_payloads() -> list[dict[str, Any]]:
    return [template.payload() for template in TEMPLATES]


def get_template(key: object) -> PromptTemplate:
    template = _TEMPLATES_BY_KEY.get(str(key or "").strip())
    if not template:
...
# Database-backed template catalog. The source templates above remain as a safe
# fallback for old databases and are also used by the seeded built-ins.
...
def _field_from_payload(raw: object) -> TemplateField:
    ...
    try:
        max_length = int(raw.get("maxLength", 180))
...
def _template_from_row(row) -> PromptTemplate:
    ...
    if source.startswith("__builtin__:") and static:
        def renderer(values: dict[str, str], builtin=static) -> str:
            # Keep legacy built-ins safe if an administrator adjusts their
            # visible field list; missing source fields become empty strings.
...
def _template_values(payload: dict[str, Any], *, current=None) -> dict[str, Any]:
    ...
    if template_type == "skill" and not skill_key:
        raise TemplateError("Skill 模板需要选择 Skill")
